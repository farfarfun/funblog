...
class BlogConversionError(ValueError):
    """无法将本地文档转换为 HTML 时抛出的领域异常。"""
...
(org-export-as-html-batch))'"""
    ...
    handlers = dict(
        htm=html2html, html=html2html, adoc=adoc2html, asciidoc=adoc2html, org=org2html
    )
    f = handlers.get(get_ext(path))
    if not f:
        raise BlogConversionError("do not know how to make html from %s" % path)
...
def resolve_local_ref(content, upload, base_dir):
    ...
    def new_img(m):
        return '<img src="%s" %s/>' % (
            cached_call(url, realpath(m.group(1))),
            m.group(2),
        )
    ...
    def resolve_img_ref(content):
        return re.sub(r'<img\s+src\s*=\s*"(.*?)"(.*?)/>', new_img, content)

    def resolve_archor_ref(content):
        return re.sub(r'<a\s+href\s*=\s*"(.*?)"(.*?)>', new_archor, content)
...
class MetaWeblog:
    ...
    def getRecentPosts(self, count=5, blogid=""):
        return self.server.metaWeblog.getRecentPosts(
            blogid, self.usr, self.passwd, count
        )

    def deletePost(self, id):
        return self.server.blogger.deletePost(
            self.appKey, id, self.usr, self.passwd, False
        )
    ...
    def newMediaObject(self, path, name=None, blogid=""):
        ...
        return self.server.metaWeblog.newMediaObject(
            blogid, self.usr, self.passwd, dict(name=name, type=type, bits=content)
        )

    def post(self, path):
        def get_tagged_body(html, tag):
            tag_re = "%s|%s" % (tag.lower(), tag.upper())
            m = re.search("<(?:%s)>(.*?)</(?:%s)>" % (tag_re, tag_re), html, re.DOTALL)
        ...
        description = resolve_local_ref(
            get_tagged_body(content, "body") or content,
            self.newMediaObject,
            os.path.dirname(os.path.realpath(path)),
        )
        matched = filter(
            lambda p: p["title"] == title.decode("utf-8"), self.getRecentPosts(10)
        )
        if matched:
    ...
    def __repr__(self):
        return f"MetaWeblog({self.serviceUrl!r}, {self.usr!r})"
