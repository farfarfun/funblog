...
def get_all_file(path_root: str) -> FileTree:
    """扫描目录并构建分类与文章文件树。"""
...
class BlogManage:
    ...
    def __init__(self, path_root: str, db_path: str | None = None) -> None:
        """初始化管理器。"""
    ...
    def insert_cate(
        self, tree: FileTree, parent_info: dict[str, Any]
    ) -> dict[str, Any]:
        """写入或更新一个分类并返回数据库记录。"""
        properties = {"describe": tree.name}
        condition = {"cate_name": tree.name, "parent_id": parent_info["cate_id"]}
        properties.update(condition)
        self.cate_db.update_or_insert(properties=properties, condition=condition)

        return self.cate_db.select(condition=condition)[0]

    def insert_page(
        self, properties: dict[str, Any], cate_info: dict[str, Any]
    ) -> dict[str, Any]:
        """写入或更新一篇文章并返回数据库记录。"""
        page = PageDetail()
        ...
        if page.page_uid is not None:
            ...
            if up.rowcount > 0:
                is_update = True
        if not is_update:
            condition = {"title": properties["title"], "cate_id": properties["cate_id"]}
            self.page_db.update_or_insert(properties=properties, condition=condition)

        return self.page_db.select(condition=condition)[0]

    def local_scan_category(self, tree: FileTree, parent_info: dict[str, Any]) -> None:
        """递归扫描分类及其文章。"""
    ...
    def publish_cate(self, blog: PublishBase, key: str = "cate_typecho_id") -> None:
        """发布尚未同步的分类。"""
        for cate in tqdm(self.cate_db.select_all()):
            if cate[key] <= 0:
                ...
                cate[key] = cate_id
                self.cate_db.update(cate, condition={"cate_id": cate["cate_id"]})

    def publish_page(self, blog: PublishBase, key: str = "page_typecho_id") -> None:
        """发布或更新本地文章。"""
        for page in tqdm(self.page_db.select_all()):
            ...
            if page[key] <= 0:
                ...
                page[key] = page_id
                self.page_db.update(page, condition={"page_id": page["page_id"]})
            else:
                blog.edit_page(page_id=page[key], page=_page)

    def publish_typecho(self, rpc_url: str, username: str, password: str) -> None:
        """扫描并发布全部本地内容到 Typecho。"""
        typecho = Typecho(rpc_url=rpc_url, username=username, password=password)
